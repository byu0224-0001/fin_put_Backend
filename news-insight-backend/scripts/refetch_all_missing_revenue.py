# -*- coding: utf-8 -*-
"""
DB에서 매출 비중이 없는 모든 기업들의 매출 비중 재수집 및 성공률 모니터링

기능:
1. DB에서 매출 비중이 없는 모든 기업 조회
2. DART API를 통한 매출 비중 재수집
3. 재수집 성공률 모니터링 (성공/실패 통계, 실패 원인 분석)
4. 재수집 후 HOLD 비율 변화 추적

개선 사항 (GPT/Gemini 피드백 반영):
- DRY RUN 결과를 실행 가능한 계획 리포트로 변경
- 재수집 "성공" 정의를 더 엄격하게
- Top20 전용 실패 원인 분해 리포트
- 단계별 성공률 KPI 측정
"""
import sys
import os
import json
import time
from datetime import datetime, UTC
from collections import defaultdict
from typing import Dict, List, Tuple, Optional

# Windows 환경에서 UTF-8 인코딩 설정
if sys.platform == 'win32':
    import codecs
    sys.stdout = codecs.getwriter('utf-8')(sys.stdout.buffer, 'strict')
    sys.stderr = codecs.getwriter('utf-8')(sys.stderr.buffer, 'strict')
    os.environ['PYTHONIOENCODING'] = 'utf-8'
else:
    sys.stdout.reconfigure(encoding='utf-8')
    sys.stderr.reconfigure(encoding='utf-8')

sys.path.insert(0, '.')

from app.db import SessionLocal
from app.models.company_detail import CompanyDetail
from app.models.stock import Stock
from app.models.investor_sector import InvestorSector
from app.services.dart_parser import DartParser
from app.services.llm_handler import LLMHandler
from app.services.embedding_filter import select_relevant_chunks
from app.services.sector_classifier import classify_sector_rule_based
from dotenv import load_dotenv
import logging

load_dotenv()
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

DART_API_KEY = os.getenv('DART_API_KEY')
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY')
MAX_LLM_CHARS = 50000


def get_missing_revenue_companies(db, limit: Optional[int] = None, ticker: Optional[str] = None):
    """매출 비중이 없는 모든 기업 조회 (또는 특정 티커)"""
    query = db.query(CompanyDetail, Stock).join(
        Stock, CompanyDetail.ticker == Stock.ticker
    )
    
    # 🆕 특정 티커 필터링
    if ticker:
        query = query.filter(CompanyDetail.ticker == ticker)
    else:
        # 매출 비중이 없는 경우: None, {}, 또는 빈 dict
        query = query.filter(
            (
                (CompanyDetail.revenue_by_segment == None) |
                (CompanyDetail.revenue_by_segment == {}) |
                (CompanyDetail.revenue_by_segment == '{}')
            )
        )
    
    query = query.order_by(Stock.market_cap.desc().nullslast())
    
    if limit:
        query = query.limit(limit)
    
    results = query.all()
    
    companies = []
    for detail, stock in results:
        # 🆕 CompanyDetail 존재, 티커 유효성 확인
        if not detail or not stock:
            continue
        
        companies.append({
            'ticker': detail.ticker,
            'name': stock.stock_name if stock else detail.ticker,
            'market_cap': stock.market_cap if stock else 0,
            'has_company_detail': True,
            'ticker_valid': bool(detail.ticker and len(detail.ticker) >= 4)
        })
    
    return companies


def validate_revenue_data(revenue_data: Optional[Dict]) -> Tuple[bool, str, Dict]:
    """
    🆕 재수집 "성공" 정의를 더 엄격하게 (P0-1: 품질 게이트 강화)
    
    Returns:
        (is_valid, reason, quality_meta)
    """
    if not revenue_data:
        return False, "NO_REVENUE_DATA", {}
    
    if not isinstance(revenue_data, dict):
        return False, "INVALID_FORMAT", {}
    
    if len(revenue_data) == 0:
        return False, "EMPTY_DICT", {}
    
    # 세그먼트/비중이 파싱 가능한지 확인
    valid_segments = {}
    adjustment_items = {}  # 조정항목 분리
    total_pct = 0.0
    
    for segment, pct in revenue_data.items():
        if not segment or not isinstance(segment, str):
            continue
        
        # NaN 체크
        if isinstance(pct, float) and (pct != pct or pct == float('inf') or pct == float('-inf')):
            continue
        
        # 조정항목 분리
        adjustment_keywords = ['연결조정', '내부거래제거', '단순합계', '기타(조정)', '조정', '제거']
        # 🆕 공백 제거 후 매칭 (예: '조 정' → '조정' 매칭)
        segment_normalized = segment.replace(' ', '').replace('\t', '').replace('\n', '')
        if any(keyword in segment_normalized for keyword in adjustment_keywords):
            adjustment_items[segment] = pct
            continue
        
        if isinstance(pct, (int, float)):
            if pct > 0:
                valid_segments[segment] = pct
                total_pct += pct
            elif pct < 0:
                # 음수는 조정항목으로 처리
                adjustment_items[segment] = pct
    
    if len(valid_segments) == 0:
        return False, "NO_VALID_SEGMENTS", {}
    
    # 최소 top1 세그먼트 비중이 존재하는지 확인
    if total_pct < 1.0:  # 최소 1% 이상
        return False, "TOTAL_PCT_TOO_LOW", {}
    
    # 🆕 P0-1: 합계 범위 체크 (70-130%)
    if total_pct < 70.0 or total_pct > 130.0:
        quality_meta = {
            'total_pct': total_pct,
            'segment_count': len(valid_segments),
            'has_adjustment': len(adjustment_items) > 0,
            'adjustment_items': adjustment_items
        }
        return False, f"TOTAL_PCT_OUT_OF_RANGE:{total_pct:.1f}", quality_meta
    
    # 성공 시 품질 메타데이터 반환
    quality_meta = {
        'total_pct': total_pct,
        'segment_count': len(valid_segments),
        'has_adjustment': len(adjustment_items) > 0,
        'adjustment_items': adjustment_items if adjustment_items else None
    }
    
    return True, "SUCCESS", quality_meta


def analyze_failure_cause(error_code: str, step_tracking: Dict) -> str:
    """
    🆕 실패 원인 분해 (A/B/C)
    
    Returns:
        'A': DART에서 해당 섹션/표를 못 찾음
        'B': 표/텍스트는 있는데 LLM이 구조화에 실패
        'C': 정규화/매핑에서 실패 (값은 있는데 검증 실패)
        'OTHER': 기타
    """
    if error_code in ['NO_REPORT', 'NO_SECTION']:
        return 'A'  # DART에서 해당 섹션/표를 못 찾음
    elif error_code == 'LLM_FAIL':
        return 'B'  # 표/텍스트는 있는데 LLM이 구조화에 실패
    elif error_code.startswith('VALIDATION_FAIL'):
        return 'C'  # 정규화/매핑에서 실패
    elif error_code == 'NO_REVENUE_DATA':
        # step_tracking을 보고 판단
        if step_tracking.get('llm_extract'):
            return 'B'  # LLM은 성공했지만 revenue_by_segment가 없음
        else:
            return 'A'  # LLM 자체가 실패
    else:
        return 'OTHER'


def fetch_and_update_revenue(
    db, 
    ticker: str, 
    stock_name: str, 
    dart_parser: DartParser, 
    llm_handler: LLMHandler, 
    year: Optional[int] = None,
    track_steps: bool = True
) -> Tuple[bool, str, Optional[Dict], Dict]:
    """
    매출 데이터 재수집 (단계별 성공률 추적)
    
    Returns:
        (success, error_code, revenue_data, step_tracking)
        error_code: 'SUCCESS', 'NO_REPORT', 'NO_SECTION', 'LLM_FAIL', 'NO_REVENUE_DATA', 'VALIDATION_FAIL', 'NO_DETAIL', 'ERROR'
        step_tracking: {'dart_fetch': bool, 'section_extract': bool, 'llm_extract': bool, 'revenue_extract': bool, 'validation': bool}
    """
    step_tracking = {
        'dart_fetch': False,
        'section_extract': False,
        'llm_extract': False,
        'revenue_extract': False,
        'validation': False
    }
    
    try:
        # Step 1: DART API로 섹션 추출 (year=None이면 최신 정기 공시 자동 선택)
        combined_text = dart_parser.extract_key_sections(ticker, target_year=year)
        step_tracking['dart_fetch'] = True
        
        if not combined_text:
            logger.warning(f"[{ticker}] {stock_name}: DART 섹션 추출 실패")
            return False, "NO_REPORT", None, step_tracking
        
        if len(combined_text) < 200:
            logger.warning(f"[{ticker}] {stock_name}: DART 섹션 추출 성공했지만 내용 부족 ({len(combined_text)}자)")
            return False, "NO_SECTION", None, step_tracking
        
        step_tracking['section_extract'] = True
        logger.info(f"[{ticker}] {stock_name}: DART 섹션 추출 성공 ({len(combined_text)}자)")
        
        # 🆕 하이브리드 파이프라인: HTML 파싱 → 정규식 → LLM Fallback
        revenue_data = None
        
        # Step 2-1: HTML 테이블 직접 파싱 시도 (우선 시도)
        try:
            from app.services.revenue_table_parser import extract_revenue_from_consolidated_tables
            
            # 금융사 여부 확인 (P1-4: 일원화된 함수 사용)
            is_financial = llm_handler.is_financial_company(
                company_name=stock_name,
                ticker=ticker,
                business_summary=combined_text
            )
            
            html_content = dart_parser.get_raw_html_for_revenue_sections(ticker, year=year, is_financial=is_financial)
            if html_content:
                logger.info(f"[{ticker}] {stock_name}: HTML 테이블 파싱 시도 (금융사: {is_financial})")
                revenue_data = extract_revenue_from_consolidated_tables(
                    html_content,
                    markdown_text=combined_text,
                    ticker=ticker,
                    is_financial=is_financial
                )
                
                if revenue_data and len(revenue_data) >= 2:
                    # 검증 통과 확인
                    is_valid, validation_reason, quality_meta = validate_revenue_data(revenue_data)
                    if is_valid:
                        logger.info(f"[{ticker}] {stock_name}: HTML 테이블 파싱 성공 (Fast Track) - {revenue_data}")
                        step_tracking['html_parse'] = True
                        step_tracking['revenue_extract'] = True
                        step_tracking['validation'] = True
                        # DB 업데이트는 아래 공통 로직 사용
                    else:
                        logger.warning(f"[{ticker}] {stock_name}: HTML 파싱 결과 검증 실패 - {validation_reason}, LLM Fallback")
                        revenue_data = None  # LLM Fallback으로 진행
                else:
                    logger.debug(f"[{ticker}] {stock_name}: HTML 테이블 파싱 실패 또는 세그먼트 부족, LLM Fallback")
                    revenue_data = None
        except Exception as e:
            logger.warning(f"[{ticker}] {stock_name}: HTML 테이블 파싱 오류, LLM Fallback: {e}")
            revenue_data = None
        
        # Step 2-2: LLM Fallback (HTML 파싱 실패 시)
        if not revenue_data:
            # Step 2: 임베딩 필터로 관련 청크 선택
            try:
                filtered_text = select_relevant_chunks(combined_text, ticker=ticker)
                effective_text = filtered_text if filtered_text and len(filtered_text) > 200 else combined_text
            except Exception as e:
                logger.warning(f"[{ticker}] 임베딩 필터링 실패, 원문 사용: {e}")
                effective_text = combined_text
            
            # 길이 제한
            if len(effective_text) > MAX_LLM_CHARS:
                effective_text = effective_text[:MAX_LLM_CHARS]
            
            # Step 3: LLM으로 구조화된 데이터 추출
            structured_data = llm_handler.extract_structured_data(
                effective_text,
                ticker=ticker,
                company_name=stock_name
            )
            
            if not structured_data:
                logger.warning(f"[{ticker}] {stock_name}: LLM 구조화 실패")
                return False, "LLM_FAIL", None, step_tracking
            
            step_tracking['llm_extract'] = True
            
            # Step 4: revenue_by_segment 추출
            revenue_data = structured_data.get('revenue_by_segment', {})
        
        if not revenue_data or not isinstance(revenue_data, dict) or len(revenue_data) == 0:
            logger.warning(f"[{ticker}] {stock_name}: 매출비중 데이터 없음")
            return False, "NO_REVENUE_DATA", None, step_tracking
        
        step_tracking['revenue_extract'] = True
        
        # Step 5: 🆕 엄격한 검증 (P0-1: 품질 게이트 강화)
        is_valid, validation_reason, quality_meta = validate_revenue_data(revenue_data)
        
        if not is_valid:
            logger.warning(f"[{ticker}] {stock_name}: 매출비중 검증 실패 - {validation_reason}, quality_meta: {quality_meta}")
            return False, f"VALIDATION_FAIL:{validation_reason}", None, step_tracking
        
        step_tracking['validation'] = True
        
        # 🆕 품질 메타데이터 로깅
        if quality_meta.get('has_adjustment'):
            logger.info(f"[{ticker}] {stock_name}: 조정항목 발견 - {quality_meta.get('adjustment_items')}")
        
        # 🆕 합계가 100%를 초과하는 경우 경고
        total_pct = quality_meta.get('total_pct', sum(revenue_data.values()))
        if total_pct > 100.0:
            logger.warning(f"[{ticker}] {stock_name}: 합계가 100% 초과 ({total_pct:.1f}%) - 조정항목 또는 반올림 오차 가능")
        
        # Step 6: DB 업데이트
        # 🆕 P0-B: 조정항목은 원본에 포함하여 저장 (분류기에서 제외 처리)
        # quality_meta에 조정항목 정보 저장 (향후 활용 가능)
        detail = db.query(CompanyDetail).filter(CompanyDetail.ticker == ticker).first()
        if detail:
            # 원본 revenue_data 저장 (조정항목 포함)
            detail.revenue_by_segment = revenue_data
            detail.updated_at = datetime.now(UTC)
            
            # 🆕 quality_meta 정보는 classification_meta에 저장 (향후 구현)
            # 현재는 로깅만 수행
            
            db.commit()
            logger.info(f"[{ticker}] {stock_name}: 매출비중 업데이트 완료 ({len(revenue_data)}개 세그먼트, 총 {sum(revenue_data.values()):.1f}%)")
            if quality_meta.get('has_adjustment'):
                logger.info(f"[{ticker}] {stock_name}: 조정항목 포함 저장 - {quality_meta.get('adjustment_items')}")
            return True, "SUCCESS", revenue_data, step_tracking
        else:
            logger.warning(f"[{ticker}] {stock_name}: CompanyDetail 없음")
            return False, "NO_DETAIL", None, step_tracking
            
    except Exception as e:
        logger.error(f"[{ticker}] {stock_name}: 오류 - {e}")
        import traceback
        traceback.print_exc()
        return False, f"ERROR: {str(e)}", None, step_tracking


def get_hold_statistics(db) -> Dict:
    """HOLD 비율 통계 계산"""
    try:
        # 전체 HOLD 비율 (실제 DB 기준)
        all_sectors = db.query(InvestorSector).filter(
            InvestorSector.is_primary == True
        ).all()
        
        hold_count = sum(1 for s in all_sectors if s.confidence and s.confidence.startswith('HOLD'))
        total = len(all_sectors)
        hold_ratio = (hold_count / total * 100) if total > 0 else 0
        
        # Top200 HOLD 비율
        top200_stocks = db.query(Stock).filter(
            Stock.market_cap >= 1000000000000  # 1조 이상
        ).order_by(Stock.market_cap.desc().nullslast()).limit(200).all()
        
        top200_tickers = [s.ticker for s in top200_stocks]
        top200_sectors = db.query(InvestorSector).filter(
            InvestorSector.is_primary == True,
            InvestorSector.ticker.in_(top200_tickers)
        ).all()
        
        top200_hold_count = sum(1 for s in top200_sectors if s.confidence and (
            s.confidence.startswith('HOLD') or 'HOLD:' in s.confidence
        ))
        top200_total = len(top200_sectors)
        top200_hold_ratio = (top200_hold_count / top200_total * 100) if top200_total > 0 else 0
        
        return {
            'overall': {
                'hold_count': hold_count,
                'total': total,
                'hold_ratio': hold_ratio
            },
            'top200': {
                'hold_count': top200_hold_count,
                'total': top200_total,
                'hold_ratio': top200_hold_ratio
            }
        }
    except Exception as e:
        logger.error(f"HOLD 통계 계산 오류: {e}")
        return {
            'overall': {'hold_count': 0, 'total': 0, 'hold_ratio': 0},
            'top200': {'hold_count': 0, 'total': 0, 'hold_ratio': 0}
        }


def refetch_all_missing_revenue(
    dry_run: bool = True,
    limit: Optional[int] = None,
    batch_size: int = 50,
    year: Optional[int] = None,
    ticker: Optional[str] = None
) -> Dict:
    """
    매출 비중이 없는 모든 기업 재수집 및 모니터링
    
    Args:
        dry_run: DRY RUN 모드 (실제 수정하지 않음)
        limit: 처리할 기업 수 제한 (None이면 전체)
        batch_size: 배치 처리 크기
        year: 대상 연도 (None이면 최신 정기 공시 자동 선택)
    """
    db = SessionLocal()
    
    if not DART_API_KEY:
        print("\n❌ DART_API_KEY 환경변수가 설정되지 않았습니다.", flush=True)
        return None
    
    if not OPENAI_API_KEY:
        print("\n❌ OPENAI_API_KEY 환경변수가 설정되지 않았습니다.", flush=True)
        return None
    
    dart_parser = DartParser(DART_API_KEY)
    llm_handler = LLMHandler()
    
    try:
        print("=" * 80, flush=True)
        print("매출 비중 없는 기업 재수집 및 성공률 모니터링", flush=True)
        print("=" * 80, flush=True)
        
        if dry_run:
            print("\n⚠️  DRY RUN 모드 (실제 수정하지 않음)", flush=True)
        else:
            print("\n✅ 실제 수정 모드", flush=True)
        
        # 매출 비중 없는 기업 조회
        print("\n[1단계] 매출 비중 없는 기업 조회 중...", flush=True)
        missing_companies = get_missing_revenue_companies(db, limit=limit, ticker=ticker)
        
        print(f"\n[대상 기업]", flush=True)
        if ticker:
            print(f"  특정 티커: {ticker}", flush=True)
        print(f"  매출 비중 없는 기업: {len(missing_companies)}개", flush=True)
        if limit:
            print(f"  처리 제한: {limit}개", flush=True)
        if year:
            print(f"  대상 연도: {year}년", flush=True)
        else:
            print(f"  대상 연도: 최신 정기 공시 자동 선택", flush=True)
        
        if not missing_companies:
            print("\n✅ 매출 비중이 필요한 기업이 없습니다.", flush=True)
            return None
        
        # 🆕 DRY RUN: 실행 가능한 계획 리포트 생성 후 즉시 반환
        if dry_run:
            print("\n[DRY RUN 계획 리포트]", flush=True)
            
            # CompanyDetail 존재, 티커 유효성 확인
            eligible_count = sum(1 for c in missing_companies if c.get('has_company_detail') and c.get('ticker_valid'))
            will_call_dart_count = eligible_count  # 모든 eligible 기업에 대해 DART 호출 예정
            will_call_llm_count = eligible_count  # DART 성공 시 LLM 호출 예정
            
            # 예상 비용 계산 (대략적)
            estimated_llm_calls = will_call_llm_count
            estimated_cost_usd = estimated_llm_calls * 0.01  # 대략 $0.01 per call
            
            print(f"  Eligible 기업 (CompanyDetail 존재, 티커 유효): {eligible_count}개", flush=True)
            print(f"  예상 DART API 호출: {will_call_dart_count}회", flush=True)
            print(f"  예상 LLM API 호출: {will_call_llm_count}회", flush=True)
            print(f"  예상 비용: 약 ${estimated_cost_usd:.2f}", flush=True)
            
            # 우선순위 표시
            top20 = missing_companies[:20]
            print(f"\n[우선 재수집 대상 (시가총액 상위 20개)]", flush=True)
            for i, company in enumerate(top20, 1):
                market_cap = company.get('market_cap', 0)
                if market_cap >= 1000000000000:
                    market_cap_str = f"{market_cap/1000000000000:.1f}조원"
                else:
                    market_cap_str = f"{market_cap/1000000000:.1f}억원"
                print(f"  {i}. {company['name']} ({company['ticker']}) - {market_cap_str}", flush=True)
            
            # DRY RUN 결과 저장
            plan_report = {
                'generated_at': datetime.now().isoformat(),
                'dry_run': True,
                'year': year,
                'planning': {
                    'total_target': len(missing_companies),
                    'eligible_count': eligible_count,
                    'will_call_dart_count': will_call_dart_count,
                    'will_call_llm_count': will_call_llm_count,
                    'estimated_cost_usd': estimated_cost_usd,
                    'top20_priorities': top20
                }
            }
            
            os.makedirs('reports', exist_ok=True)
            output_file = 'reports/refetch_revenue_plan.json'
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(plan_report, f, ensure_ascii=False, indent=2, default=str)
            
            print(f"\n✅ 계획 리포트 저장: {output_file}", flush=True)
            print(f"\n⚠️  실제 재수집을 원하시면 --apply 플래그를 사용하세요:", flush=True)
            print(f"  python scripts/refetch_all_missing_revenue.py --apply --limit 20", flush=True)
            print("=" * 80, flush=True)
            
            return plan_report
        
        # 실제 재수집 모드에서만 아래 코드 실행
        # 재수집 전 HOLD 비율 확인
        print("\n[2단계] 재수집 전 HOLD 비율 확인 중...", flush=True)
        before_stats = get_hold_statistics(db)
        
        print(f"  전체 HOLD 비율: {before_stats['overall']['hold_ratio']:.1f}% ({before_stats['overall']['hold_count']}/{before_stats['overall']['total']})", flush=True)
        print(f"  Top200 HOLD 비율: {before_stats['top200']['hold_ratio']:.1f}% ({before_stats['top200']['hold_count']}/{before_stats['top200']['total']})", flush=True)
        
        # 재수집 진행
        print("\n[3단계] 매출 비중 재수집 진행 중...", flush=True)
        
        results = []
        success_count = 0
        fail_count = 0
        error_stats = defaultdict(int)
        failure_cause_stats = defaultdict(int)  # A/B/C 분포
        step_success_stats = defaultdict(int)  # 단계별 성공률
        
        # Top20 전용 통계
        top20_results = []
        top20_success_count = 0
        top20_fail_count = 0
        top20_failure_cause_stats = defaultdict(int)
        
        total = len(missing_companies)
        if limit:
            total = min(total, limit)
        
        for idx, company in enumerate(missing_companies[:limit] if limit else missing_companies, 1):
            ticker = company['ticker']
            name = company['name']
            market_cap = company.get('market_cap', 0)
            
            print(f"\n[{idx}/{total}] {name} ({ticker}) 재수집 중...", flush=True)
            
            if dry_run:
                # DRY RUN: 시뮬레이션만
                print(f"  [DRY RUN] 재수집 예정", flush=True)
                results.append({
                    'ticker': ticker,
                    'name': name,
                    'market_cap': market_cap,
                    'status': 'DRY_RUN',
                    'error_code': None,
                    'revenue_data': None
                })
                success_count += 1  # DRY RUN에서는 모두 성공으로 카운트
            else:
                # 실제 재수집
                success, error_code, revenue_data, step_tracking = fetch_and_update_revenue(
                    db, ticker, name, dart_parser, llm_handler, year, track_steps=True
                )
                
                # 단계별 성공률 추적
                for step, step_success in step_tracking.items():
                    if step_success:
                        step_success_stats[step] += 1
                
                is_top20 = idx <= 20
                
                if success:
                    success_count += 1
                    segment_count = len(revenue_data) if revenue_data else 0
                    total_pct = sum(revenue_data.values()) if revenue_data else 0
                    
                    results.append({
                        'ticker': ticker,
                        'name': name,
                        'market_cap': market_cap,
                        'status': 'SUCCESS',
                        'error_code': error_code,
                        'revenue_data': revenue_data,
                        'segment_count': segment_count,
                        'total_pct': total_pct,
                        'step_tracking': step_tracking
                    })
                    print(f"  ✅ 성공: {segment_count}개 세그먼트, 총 {total_pct:.1f}%", flush=True)
                    
                    if is_top20:
                        top20_success_count += 1
                        top20_results.append(results[-1])
                else:
                    fail_count += 1
                    error_stats[error_code] += 1
                    
                    # 실패 원인 분해
                    failure_cause = analyze_failure_cause(error_code, step_tracking)
                    failure_cause_stats[failure_cause] += 1
                    
                    results.append({
                        'ticker': ticker,
                        'name': name,
                        'market_cap': market_cap,
                        'status': 'FAIL',
                        'error_code': error_code,
                        'revenue_data': None,
                        'step_tracking': step_tracking,
                        'failure_cause': failure_cause
                    })
                    print(f"  ❌ 실패: {error_code} (원인: {failure_cause})", flush=True)
                    
                    if is_top20:
                        top20_fail_count += 1
                        top20_failure_cause_stats[failure_cause] += 1
                        top20_results.append(results[-1])
                
                # Rate limit 방지
                if idx < total:
                    time.sleep(1)
            
            # 진행률 출력
            if idx % 10 == 0 or idx == total:
                progress = (idx / total * 100) if total > 0 else 0
                print(f"\n  진행: {idx}/{total} ({progress:.1f}%)", flush=True)
        
        # 재수집 후 HOLD 비율 확인 (실제 수정 모드에서만)
        if not dry_run:
            print("\n[4단계] 재수집 후 HOLD 비율 확인 중...", flush=True)
            after_stats = get_hold_statistics(db)
            
            print(f"  전체 HOLD 비율: {after_stats['overall']['hold_ratio']:.1f}% ({after_stats['overall']['hold_count']}/{after_stats['overall']['total']})", flush=True)
            print(f"  Top200 HOLD 비율: {after_stats['top200']['hold_ratio']:.1f}% ({after_stats['top200']['hold_count']}/{after_stats['top200']['total']})", flush=True)
        else:
            after_stats = before_stats  # DRY RUN에서는 동일하게 설정
        
        # 통계 계산
        total_processed = len(results)
        success_rate = (success_count / total_processed * 100) if total_processed > 0 else 0
        
        # 단계별 성공률 계산 (실제 재수집 모드에서만)
        if not dry_run:
            step_success_rates = {
                step: (count / total_processed * 100) if total_processed > 0 else 0
                for step, count in step_success_stats.items()
            }
        else:
            step_success_rates = {}
        
        # Top20 성공률 (실제 재수집 모드에서만)
        if not dry_run:
            top20_success_rate = (top20_success_count / 20 * 100) if top20_success_count + top20_fail_count > 0 else 0
        else:
            top20_success_rate = 0
        
        # 결과 리포트 생성
        report = {
            'generated_at': datetime.now().isoformat(),
            'dry_run': dry_run,
            'year': year,
            'statistics': {
                'total_target': len(missing_companies),
                'total_processed': total_processed,
                'success_count': success_count,
                'fail_count': fail_count,
                'success_rate': success_rate if not dry_run else "N/A (DRY RUN)",
                'error_distribution': dict(error_stats) if not dry_run else {},
                'failure_cause_distribution': dict(failure_cause_stats) if not dry_run else {},  # 🆕 A/B/C 분포
                'step_success_rates': step_success_rates  # 🆕 단계별 성공률
            },
            'top20_analysis': {  # 🆕 Top20 전용 분석 (실제 재수집 모드에서만)
                'success_count': top20_success_count if not dry_run else 0,
                'fail_count': top20_fail_count if not dry_run else 0,
                'success_rate': top20_success_rate if not dry_run else 0,
                'failure_cause_distribution': dict(top20_failure_cause_stats) if not dry_run else {},
                'results': top20_results if not dry_run else []
            },
            'hold_statistics': {
                'before': before_stats,
                'after': after_stats,
                'improvement': {
                    'overall_hold_ratio_change': after_stats['overall']['hold_ratio'] - before_stats['overall']['hold_ratio'],
                    'top200_hold_ratio_change': after_stats['top200']['hold_ratio'] - before_stats['top200']['hold_ratio']
                }
            },
            'results': results
        }
        
        # 리포트 출력
        print("\n" + "=" * 80, flush=True)
        print("재수집 결과", flush=True)
        print("=" * 80, flush=True)
        
        print(f"\n[처리 통계]", flush=True)
        print(f"  대상 기업: {len(missing_companies)}개", flush=True)
        print(f"  처리 완료: {total_processed}개", flush=True)
        if dry_run:
            print(f"  성공: {success_count}개 (DRY RUN: 모두 재수집 예정)", flush=True)
            print(f"  실패: {fail_count}개", flush=True)
            print(f"  성공률: N/A (DRY RUN 모드에서는 실제 성공률을 알 수 없음)", flush=True)
            print(f"  예상 성공률: 50-70% (과거 경험치 기준)", flush=True)
        else:
            print(f"  성공: {success_count}개", flush=True)
            print(f"  실패: {fail_count}개", flush=True)
            print(f"  성공률: {success_rate:.1f}%", flush=True)
        
        if not dry_run:
            print(f"\n[단계별 성공률]", flush=True)
            if step_success_rates:
                for step, rate in step_success_rates.items():
                    print(f"  {step}: {rate:.1f}%", flush=True)
            else:
                print(f"  (데이터 없음)", flush=True)
            
            if error_stats:
                print(f"\n[실패 원인 분석]", flush=True)
                for error_code, count in sorted(error_stats.items(), key=lambda x: x[1], reverse=True):
                    print(f"  {error_code}: {count}개", flush=True)
            
            print(f"\n[실패 원인 분해 (A/B/C)]", flush=True)
            print(f"  A (DART에서 섹션/표 못 찾음): {failure_cause_stats.get('A', 0)}개", flush=True)
            print(f"  B (LLM 구조화 실패): {failure_cause_stats.get('B', 0)}개", flush=True)
            print(f"  C (검증 실패): {failure_cause_stats.get('C', 0)}개", flush=True)
            print(f"  기타: {failure_cause_stats.get('OTHER', 0)}개", flush=True)
            
            print(f"\n[Top20 분석]", flush=True)
            print(f"  성공: {top20_success_count}개", flush=True)
            print(f"  실패: {top20_fail_count}개", flush=True)
            print(f"  성공률: {top20_success_rate:.1f}%", flush=True)
            if top20_failure_cause_stats:
                print(f"  실패 원인 분해:", flush=True)
                for cause, count in top20_failure_cause_stats.items():
                    print(f"    {cause}: {count}개", flush=True)
        
        print(f"\n[HOLD 비율 변화]", flush=True)
        if not dry_run:
            print(f"  전체 HOLD 비율: {before_stats['overall']['hold_ratio']:.1f}% → {after_stats['overall']['hold_ratio']:.1f}% (Δ{after_stats['overall']['hold_ratio'] - before_stats['overall']['hold_ratio']:+.1f}%p)", flush=True)
            print(f"  Top200 HOLD 비율: {before_stats['top200']['hold_ratio']:.1f}% → {after_stats['top200']['hold_ratio']:.1f}% (Δ{after_stats['top200']['hold_ratio'] - before_stats['top200']['hold_ratio']:+.1f}%p)", flush=True)
        else:
            print(f"  (DRY RUN 모드: 실제 HOLD 비율 변화는 --apply 실행 후 확인 가능)", flush=True)
        
        # 리포트 저장
        os.makedirs('reports', exist_ok=True)
        output_file = 'reports/refetch_revenue_monitoring.json'
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2, default=str)
        
        print(f"\n✅ 결과 저장: {output_file}", flush=True)
        
        if dry_run:
            print(f"\n⚠️  실제 재수집을 원하시면 --apply 플래그를 사용하세요:", flush=True)
            print(f"  python scripts/refetch_all_missing_revenue.py --apply", flush=True)
        
        print("=" * 80, flush=True)
        
        return report
        
    except Exception as e:
        import traceback
        print(f"\n❌ 오류 발생: {e}", flush=True)
        traceback.print_exc()
        raise
    finally:
        db.close()


def main():
    import argparse
    
    parser = argparse.ArgumentParser(description='매출 비중 없는 기업 재수집 및 모니터링')
    parser.add_argument('--apply', action='store_true', help='실제 수정 모드 (기본값: DRY RUN)')
    parser.add_argument('--limit', type=int, default=None, help='처리할 기업 수 제한')
    parser.add_argument('--batch-size', type=int, default=50, help='배치 처리 크기')
    parser.add_argument('--year', type=int, default=None, help='대상 연도 (지정하지 않으면 최신 정기 공시 자동 선택)')
    
    args = parser.parse_args()
    
    refetch_all_missing_revenue(
        dry_run=not args.apply,
        limit=args.limit,
        batch_size=args.batch_size,
        year=args.year
    )


if __name__ == '__main__':
    main()

